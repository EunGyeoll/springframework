...
public class Ch02AuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, 
		...
		//요청 매핑 메소드 참조 객체
		HandlerMethod handlerMethod = (HandlerMethod) handler; //강제 타입 변환
		 
		//요청 매핑 메소드에 @Auth가 붙어있지 않다면 null
		//요청 매핑 메소드에 @Auth 붙어있다면 not null
		...
		if(auth == null) {
			...
			if(loginStatus)	{
				...
				response.sendRedirect(request.getContextPath()); // "/" 쓰면 localhost:8080까지를 의미함
				//요청 매핑 메소드 실행을 하지 않음
				return false;
			}
		}
